...
class CssBoxWidgetWithInlineSpanChildren extends StatelessWidget {
  const CssBoxWidgetWithInlineSpanChildren(
      {super.key,
...
class CssBoxWidget extends StatelessWidget {
    ...
    return _CSSBoxRenderer(
      ...
      children: [
        Container(
          ...
          width: _shouldExpandToFillBlock(style) ? double.infinity : null,
          padding: padding,
          child: top
              ? child
              : MediaQuery(
  ...
  bool _shouldExpandToFillBlock(Style style) {
...
InlineSpan? _generateMarkerBoxSpan(Style style, MarkManager markManager) {
...
Widget _generateWidgetChild(BuildContext context, List<InlineSpan> children, Style style,
  ...
  if (style.listStylePosition == ListStylePosition.inside) {
    final inlineMarkerBox = _generateMarkerBoxSpan(style, markManager);
...
class RenderCSSBox extends RenderBox
  ...
  static double getIntrinsicDimension(
      RenderBox? firstChild, double Function(RenderBox child) mainChildSizeGetter) {
    double extent = 0.0;
  ...
  void performLayout() {
    ...
    if (markerBox != null) {
      ...
      final distance =
          (child.getDistanceToBaseline(TextBaseline.alphabetic, onlyReal: true) ?? 0) + topOffset;
      final offsetHeight = distance -
          (markerBox.getDistanceToBaseline(TextBaseline.alphabetic) ?? markerBox.size.height);
